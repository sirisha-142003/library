...
def update_record():
    ...
    edit = Button(left_frame, text='Update Record', font=btn_font, bg=btn_bg, width=20, command=update)
...
# Variables
lf_bg = '#A7D8D8'  # Left Frame Background Color (light teal)
rtf_bg = '#7FB5B5'  # Right Top Frame Background Color (light greenish-blue)
rbf_bg = '#66A3A3'  # Right Bottom Frame Background Color (medium teal)
btn_bg = '#66A3A3'  # Medium teal background color for buttons and headers
btn_font_color = '#FFFFFF'  # Button Text Color (white for contrast)
entry_bg = '#FFFFFF'  # Entry Background Color (white for clean look)
entry_fg = '#2F4F4F'  # Entry Text Color (dark grayish green)
tree_bg = '#66A3A3'  # Tree Background Color (medium teal)

lbl_font = ('Georgia', 13, 'bold')  # Font for all labels
...
Label(root, text='LIBRARY MANAGEMENT SYSTEM', font=("Noto Sans CJK TC", 15, 'bold'), bg=btn_bg, fg=btn_font_color).pack(side=TOP, fill=X)
...
# Left Frame
Label(left_frame, text='Book Name', bg=lf_bg, font=lbl_font, fg=btn_font_color).place(x=98, y=25)
Entry(left_frame, width=25, font=entry_font, text=bk_name, bg=entry_bg, fg=entry_fg).place(x=45, y=55)

Label(left_frame, text='Book ID', bg=lf_bg, font=lbl_font, fg=btn_font_color).place(x=110, y=105)
bk_id_entry = Entry(left_frame, width=25, font=entry_font, text=bk_id, bg=entry_bg, fg=entry_fg)
bk_id_entry.place(x=45, y=135)

Label(left_frame, text='Author Name', bg=lf_bg, font=lbl_font, fg=btn_font_color).place(x=90, y=185)
Entry(left_frame, width=25, font=entry_font, text=author_name, bg=entry_bg, fg=entry_fg).place(x=45, y=215)

Label(left_frame, text='Status of the Book', bg=lf_bg, font=lbl_font, fg=btn_font_color).place(x=75, y=265)
dd = OptionMenu(left_frame, bk_status, *['Available', 'Issued'])
dd.configure(font=entry_font, width=12, bg='gray15', fg='white')
dd.place(x=75, y=300)

submit = Button(left_frame, text='Add new record', font=btn_font, bg=btn_bg, fg=btn_font_color, width=20, command=add_record)
submit.place(x=50, y=375)

clear = Button(left_frame, text='Clear fields', font=btn_font, bg=btn_bg, fg=btn_font_color, width=20, command=clear_fields)
clear.place(x=50, y=435)

# Right Top Frame
Button(RT_frame, text='Delete book record', font=btn_font, bg=btn_bg, fg=btn_font_color, width=17, command=remove_record).place(x=8, y=30)
Button(RT_frame, text='Delete full inventory', font=btn_font, bg=btn_bg, fg=btn_font_color, width=17, command=delete_inventory).place(x=178, y=30)
Button(RT_frame, text='Update book details', font=btn_font, bg=btn_bg, fg=btn_font_color, width=17,
       command=update_record).place(x=348, y=30)
Button(RT_frame, text='Change Book Availability', font=btn_font, bg=btn_bg, fg=btn_font_color, width=19,
       command=change_availability).place(x=518, y=30)

# Right Bottom Frame
Label(RB_frame, text='BOOK INVENTORY', bg=rbf_bg, font=("Noto Sans CJK TC", 15, 'bold'), fg='black').pack(side=TOP, fill=X)

tree = ttk.Treeview(RB_frame, selectmode=BROWSE, columns=('Book Name', 'Book ID', 'Author', 'Status', 'Issuer Card ID'), style='Treeview')
